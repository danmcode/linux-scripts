...
def backup_database():
    ...
    backup_file = os.path.join(
        BACKUP_DIR,
        f"backup_{DB_NAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.sql"
    )

    ignore_tables_cmd = ' '.join(
        f"--ignore-table={DB_NAME}.{table.strip()}" for table in IGNORE_TABLES if table.strip()
    )
    comando = f"mysqldump -u {DB_USER} -p'{DB_PASSWORD}' {DB_NAME} {ignore_tables_cmd} > {backup_file}"
...
def send_email(message, subject=None, emails_cc=None):
    ...
    msg['Subject'] = subject or f"BACKUP DB {DB_NAME}"
...
def main():
    try:
        ...
        if not os.path.exists(NETWORK_DRIVE):
            message = f"ERROR: La unidad de red {NETWORK_DRIVE} no está montada o no existe."
            send_email(message, subject=f"ERROR BACKUP DB {DB_NAME}")
            return
        ...
        message = f"✅ Se ha realizado una copia de seguridad de la base de datos {DB_NAME} en {fecha_realizacion}."
        send_email(message)

    except Exception as e:
        fecha_realizacion = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        message = f"❌ No se pudo realizar la copia de seguridad de {DB_NAME} en {fecha_realizacion}:\n{e}"
        send_email(message, subject=f"ERROR BACKUP DB {DB_NAME}")
